...
var $smartscan_textbox = generateTextboxTM(
	"SmartScan",
	"Bringing the power and versatility of AFM technology to everyone"
);
...
jQuery("area").each(function () {
	...
	$div.appendTo(`.tooltip-section .${tooltip_div}`);
	$hotspot.clone().appendTo(`.${class_name}-map-wrap`);
  // pc
	eval("$" + model_name + "_textbox").appendTo(`.${class_name}-map-wrap`);
  // mobile
  eval("$" + model_name + "_textbox").appendTo(`.tooltip-m-section .tooltip-${model_name}`);
	i = i + 1;
});
